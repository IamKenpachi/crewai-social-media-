...
import { DEFAULT_CLIPS, DEFAULT_SONG_LYRICS } from './src/data/demoContent.js';
import { parseLyriaLyricsToSubtitles } from './src/utils/liricleParser.js';
...
function resolveGeminiModelInfo(modelName?: string): { apiModel: string; displayName: string } {
  ...
  const exactMap: Record<string, { apiModel: string; displayName: string }> = {
    'gemini-3.7-flash': { apiModel: 'gemini-3.7-flash', displayName: 'Gemini 3.7 Flash' },
    'gemini-3.6-flash': { apiModel: 'gemini-3.6-flash', displayName: 'Gemini 3.6 Flash' },
    'gemini-3.5-flash': { apiModel: 'gemini-3.5-flash', displayName: 'Gemini 3.5 Flash' },
    'gemini-3.1-flash-lite': { apiModel: 'gemini-3.1-flash-lite', displayName: 'Gemini 3.5 Flash Lite' },
    'gemini-3.1-pro-preview': { apiModel: 'gemini-3.1-pro-preview', displayName: 'Gemini 3.1 Pro' },
  };

  if (exactMap[clean]) return exactMap[clean];

  if (clean === '3.7' || clean.includes('3.7')) {
    return { apiModel: 'gemini-3.7-flash', displayName: 'Gemini 3.7 Flash' };
  }
  if (clean === '3.6' || clean.includes('3.6')) {
    return { apiModel: 'gemini-3.6-flash', displayName: 'Gemini 3.6 Flash' };
  }
  if (clean.includes('lite') || clean.includes('3.5 flash lite') || clean.includes('3.5-flash-lite')) {
    return { apiModel: 'gemini-3.1-flash-lite', displayName: 'Gemini 3.5 Flash Lite' };
  }
  if (clean === '3.5' || clean.includes('3.5-flash') || clean.includes('3.5 flash')) {
...
Return a structured JSON object strictly conforming to the requested schema.`;
    ...
    // Ensure extracted_clips always has valid items
    if (!briefResult.extracted_clips || briefResult.extracted_clips.length === 0) {
      briefResult.extracted_clips = DEFAULT_CLIPS;
    }
...
Return strictly JSON conforming to the schema.`;
    ...
    const audioTask = async () => {
      ...
      try {
        ...
        musicOutput = {
          ...
          duration_seconds: 15,
          lyrics_progression: DEFAULT_SONG_LYRICS
        };
      }
    ...
    const serverParsedLyrics = audioRes.output?.lyrics 
      ? parseLyriaLyricsToSubtitles(audioRes.output.lyrics, audioRes.output?.duration_seconds || 27) 
      : [];
...
app.post('/api/generate-thumbnail', async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      console.warn('gemini-3-pro-image regeneration note:', e?.message);
    }

    const isAiGenerated = !!thumbnailUrl;
    ...
    res.json({ success: true, thumbnailUrl, is_ai_generated: isAiGenerated, is_fallback: !isAiGenerated });
  } catch (err: any) {
