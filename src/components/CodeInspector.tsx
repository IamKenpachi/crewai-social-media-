    ...
    <div id="code-inspector-root" className="w-full flex flex-col gap-6">
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-6 relative overflow-hidden shadow-xs">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={handleCopy}
              className="flex items-center gap-1.5 px-4 py-2.5 rounded-xl bg-white hover:bg-slate-50 text-slate-700 text-xs font-bold border border-slate-200 shadow-xs transition-all cursor-pointer"
            >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-3 flex flex-col gap-2">
          <div className="p-3 bg-white border border-slate-200 rounded-xl text-xs font-bold text-slate-700 uppercase tracking-wider flex items-center gap-2 shadow-xs">
            ...
            {CREWAI_CODE_SNIPPETS.map((file) => (
              <button
                ...
                className={`flex flex-col p-3 rounded-xl text-left border transition-all cursor-pointer ${
                  selectedFile === file.filename
                    ? 'bg-blue-50/80 border-blue-600 text-blue-950 shadow-xs ring-1 ring-blue-600 font-medium'
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[10px] uppercase font-mono px-1.5 py-0.5 rounded bg-slate-100 border border-slate-200 text-slate-600 font-bold">
                    {file.language}
                  </span>
                </div>
          ...
          <div className="mt-2 p-3.5 rounded-xl bg-white border border-slate-200 text-xs shadow-xs">
            <span className="text-slate-400 font-bold text-[11px] uppercase tracking-wider block mb-1.5">Quick Install:</span>
            <code className="text-emerald-400 font-mono text-[11px] block bg-slate-900 p-2.5 rounded-lg break-all border border-slate-800">
        ...
        <div className="lg:col-span-9 flex flex-col gap-3">
          <div className="p-3.5 rounded-xl bg-white border border-slate-200 flex items-center justify-between text-xs shadow-xs">
            <div className="flex items-center gap-2">
              ...
              <span className="font-mono font-bold text-slate-900">{currentSnippet.filename}</span>
              <span className="text-slate-500">— {currentSnippet.description}</span>
            </div>
