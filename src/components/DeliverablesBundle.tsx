      ...
      {activeTab === 'video' && (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-5 flex flex-col gap-4">
            ...
            {/* Lyria Song Lyrics (Pure Non-Interactive Text Output on Top of Soundtrack Master) */}
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-xs flex flex-col gap-3">
              <div className="flex items-center justify-between pb-2 border-b border-slate-100 dark:border-slate-800">
                <span className="text-[11px] font-bold text-slate-400 uppercase tracking-widest flex items-center gap-1.5">
                  ...
                  <span>Lyria Song Lyrics</span>
                </span>
                <button
                  onClick={() => {
                    const lyricsText = bundle.music_metadata?.lyrics || currentSubtitles.map(l => l.text).join('\n');
                    handleCopy(lyricsText, 'lyrics-text');
                  }}
                  className="flex items-center gap-1 px-2.5 py-1 rounded-md bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:text-blue-600 text-[11px] font-bold cursor-pointer transition-colors"
                >
                  {copiedKey === 'lyrics-text' ? (
                    <>
              ...
              {/* Pure Text Lyrics Output (Clean Preformatted Lyrics) */}
              <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 text-xs font-mono text-slate-800 dark:text-slate-200 leading-relaxed whitespace-pre-line max-h-60 overflow-y-auto select-text">
                {bundle.music_metadata?.lyrics || currentSubtitles.map(l => (l.emoji ? `${l.text} ${l.emoji}` : l.text)).join('\n') || 'No lyrics generated for this track.'}
              </div>
            </div>
